...
/**
 * Android TV / Google TV Remote v2 transport.
 *
 * Pairing uses the official Remote v2 flow on TCP 6467 and a 6-character
 * hexadecimal code shown by the TV. After pairing, commands use TCP 6466.
 */
class AndroidTvRemote(private val context: Context, private val device: TvDevice) {
    ...
    suspend fun pairWithPrompt(requestPin: suspend () -> String): Result<Unit> = runCatching {
        ...
        try {
            ...
            require(readMessage(ssl.inputStream) != null) { "El TV no respondió a la solicitud de emparejamiento." }

            // 2. Options: hexadecimal, 6 symbols, input role.
            send(
                ssl.outputStream,
                outer(
                    20,
                    pbBytes(1, pbUInt(1, 3) + pbUInt(2, 6)) + pbUInt(3, 1)
                )
            )
            require(readMessage(ssl.inputStream) != null) { "El TV no respondió a las opciones de emparejamiento." }

            // 3. Configuration. The TV displays the 6-character hexadecimal PIN after this.
            send(
                ssl.outputStream,
                outer(
                    30,
                    pbBytes(1, pbUInt(1, 3) + pbUInt(2, 6)) + pbUInt(2, 1)
                )
            )
            require(readMessage(ssl.inputStream) != null) { "El TV no mostró el código de emparejamiento." }
            ...
            // 4. Secret = SHA-256(client RSA key + server RSA key + last 4 PIN chars).
            val secret = pairingSecret(ssl, code)
            send(ssl.outputStream, outer(40, pbBytes(1, secret)))

            // 5. SecretAck. The contents are not needed; a framed response means the TV accepted it.
            require(readMessage(ssl.inputStream) != null) {
                "El TV no confirmó el emparejamiento."
            }
    ...
    /** Clears only the pairing flag. The identity is kept so a retry can reuse the same certificate. */
    fun markNotPaired() {
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
            .edit()
            .putBoolean(KEY_PAIRED_PREFIX + device.ip, false)
            .apply()
    }
    ...
    fun connect(): Result<Unit> = runCatching {
        ...
        send(
            ...
            outerRemote(
                1,
                pbUInt(1, 622) + pbBytes(
                    2,
                    pbStr(1, "Js TV Remote") +
                        pbStr(2, "Js TV Remote") +
                        pbUInt(3, 1) +
                        pbStr(4, "1")
                )
            )
    ...
    fun sendKey(keyCode: Int): Result<Unit> = runCatching {
        ...
        // RemoteKeyInject { key_code, direction=SHORT(3) }
    ...
    private fun loadOrCreateIdentity(): KeyPair {
        ...
        if (!privateB64.isNullOrBlank() && !certB64.isNullOrBlank()) {
            try {
                ...
                // A previous app version may have stored a malformed certificate.
                // Recreate the identity instead of surfacing "X.509 not found".
                prefs.edit()
                    .remove(KEY_CERT)
                    .remove(KEY_PRIVATE)
                    .putBoolean(KEY_PAIRED_PREFIX + device.ip, false)
    ...
    /**
     * Parses DER directly through Bouncy Castle. Android's generic
     * CertificateFactory can report "X.509 not found" on some provider
     * combinations; this avoids that provider lookup entirely.
     */
    private fun certificateFrom(b64: String): X509Certificate {
        ensureProvider()
        val der = Base64.decode(b64, Base64.DEFAULT)
        return JcaX509CertificateConverter()
            .setProvider("BC")
            .getCertificate(X509CertificateHolder(der))
    }
    ...
    private fun pairingSecret(ssl: SSLSocket, pin: String): ByteArray {
        ...
        val serverCert = ssl.session.peerCertificates.firstOrNull() as? X509Certificate
            ?: error("El TV no presentó un certificado RSA válido.")

        val clientKey = clientCert.publicKey as? RSAPublicKey
            ?: error("La identidad del teléfono no usa RSA.")
        val serverKey = serverCert.publicKey as? RSAPublicKey
            ?: error("El TV no usa una clave RSA compatible.")
        ...
        val expected = pin.substring(0, 2).toInt(16)
        require((digest[0].toInt() and 0xFF) == expected) {
            "Código incorrecto. Verifica el código que aparece en el TV."
        }
        return digest
    }
            ...
            try {
                while (readerRunning.get() && !s.isClosed) {
                    ...
                    if (containsField(message, 8)) {
                        // RemotePingRequest -> RemotePingResponse { val1=1 }
                        val out = output ?: break
                        send(out, outerRemote(9, pbUInt(1, 1)))
                    }
                }
            } catch (_: Exception) {
                // The UI will reconnect on the next key press.
            } finally {
                readerRunning.set(false)
                if (socket === s) {
                    socket = null
    ...
    /** Detects a top-level protobuf field without requiring a generated protobuf runtime. */
    ...
    private fun readMessage(input: InputStream): ByteArray? {
        ...
        require(len in 0..1_048_576) { "Mensaje de TV demasiado grande." }
    ...
    private fun varint(v0: Int): ByteArray {
        var v = v0
    ...
    private fun pbBytes(field: Int, value: ByteArray) =
        varint((field shl 3) or 2) + varint(value.size) + value

    private fun pbStr(field: Int, value: String) =
        pbBytes(field, value.toByteArray(Charsets.UTF_8))

    private fun outer(field: Int, value: ByteArray) =
        pbUInt(1, 2) + pbUInt(2, 200) + pbBytes(field, value)

    private fun outerRemote(field: Int, value: ByteArray) =
        pbBytes(field, value)
